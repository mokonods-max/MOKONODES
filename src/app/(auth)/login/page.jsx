...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormError(messages[err.code] || 'حدث خطأ غير متوقع');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      setFormError('فشل تسجيل الدخول بحساب Google');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      {/* ─── Login Card ─── */}
      <GlassCard hover={false} className="relative z-10 w-full max-w-md p-8">
        {/* Logo & Title */}
        <motion.div
        ...
        <motion.button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-lg mb-6 font-medium transition-all"
          style={{
          ...
          <AnimatePresence>
            {formError && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                className="text-sm py-2 px-3 rounded-lg text-center"
                style={{
